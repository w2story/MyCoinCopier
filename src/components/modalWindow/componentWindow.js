export class ComponentWindow {
    constructor({ windowName = 'MCC::후원체크창', windowFeatures = 'width=400,height=800,resizable,scrollbars=yes,status=1', replace = false, } = {}) {
        this._windowOptions = ['http://localhost:8080/donChk', windowName, windowFeatures, replace];
    }
    get window() {
        if (!this.isOpened) {
